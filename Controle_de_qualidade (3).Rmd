...
```{r}
X_bar <- mean(tabela_pureza$Teor_Pureza)
AM_bar <- mean(na.omit(tabela_pureza$Amplitude_Movel))
...
### Gráfico AM: calcular o limite inferior e o superior

```{r}
# Calcular os limites de controle para os gráficos AM
D3 = 0 ; D4 = 3.267 

# Cálculo dos limites de controle para o gráfico AM
AM_bar <- mean(tabela_pureza2$Amplitude_Movel1, na.rm = TRUE)  # Média da Amplitude Móvel
LSC_AM <- D4 * AM_bar
LIC_AM <- D3 * AM_bar
cat("LSC AM:", LSC_AM, "\n")
cat("LIC AM:", LIC_AM, "\n\n")
...
```
### Gráfico X: calcular o limite inferior e o superior

```{r}
# Cálculo dos limites de controle para o gráfico X
d2 = 1.128

X_bar <- mean(tabela_pureza2$Teor_Pureza1, na.rm = TRUE)  # Média do Teor de Pureza
LSC_X <- X_bar + 3 * (AM_bar / d2)
LIC_X <- X_bar - 3 * (AM_bar / d2)
cat("LSC X:", LSC_X, "\n")
cat("LIC X:", LIC_X, "\n")
